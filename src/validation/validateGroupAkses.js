...
export const IsSaOperator = () => {
    const { userAplikasi } = useStateContext();
    if (userAplikasi !==null && userAplikasi.groupakses !== null && userAplikasi.groupakses === process.env.NEXT_PUBLIC_USER_SA_OPERATOR) {
        return true;
    }
    return false;
};
...
export const IsSaSupervisor = () => {
    const { userAplikasi } = useStateContext();
    if (userAplikasi !==null && userAplikasi.groupakses !== null && userAplikasi.groupakses === process.env.NEXT_PUBLIC_USER_SA_SUPERVISOR) {
        return true;
    }
    return false;
};
